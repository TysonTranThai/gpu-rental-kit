...
# =============================================================================
# mg_resolve_auto <model_gb|- > — automatic selection (current-shell state).
#   * Fits (estimated) in the largest single GPU → use just that GPU.
#   * Otherwise, if the aggregate VRAM covers it → use all available GPUs.
#   * Otherwise → fall back to the largest single GPU and warn.
# Sets MG_SELECTED, MG_SEL_COUNT, MG_AUTO_REASON.
# =============================================================================
mg_resolve_auto() {
    local model_gb="${1:-}"
    local allowed best total_needed

    mg_ensure_detection
    allowed="$(mg_allowed_ids | tr '\n' ' ' | sed 's/ *$//')"
    best="$(mg_pick_best_single)"

    if [[ -z "${model_gb}" || "${model_gb}" == "-" ]]; then
        MG_SELECTED="${best}"
        MG_SEL_COUNT=1
        MG_AUTO_REASON="Model size unknown — using the largest single GPU (safety default). Pass --size-gb for automatic multi-GPU."
        return 0
    fi

    total_needed="$(mg_needed_mb "${model_gb}")"
    if [[ "${total_needed}" -le "$(mg_max_single_vram_mb)" ]]; then
        MG_SELECTED="${best}"
        MG_SEL_COUNT=1
        MG_AUTO_REASON="Model fits in the largest single GPU."
        return 0
    fi

    if [[ "${total_needed}" -le "${GPU_TOTAL_VRAM_MB}" ]]; then
        MG_SELECTED="${allowed}"
        MG_SEL_COUNT="$(echo "${MG_SELECTED}" | wc -w | tr -d ' ')"
        MG_AUTO_REASON="Model does not fit in one GPU but fits the aggregate VRAM — using all ${MG_SEL_COUNT} GPUs."
        return 0
    fi

    MG_SELECTED="${best}"
    MG_SEL_COUNT=1
    MG_AUTO_REASON="WARNING: Model may not fit even in aggregate VRAM — using the largest single GPU."
    return 0
}
...
mg_select_summary() {
    ...
    for id in ${MG_SELECTED}; do
        out="${out}${sep}GPU ${id} ($(gpu_name_at "${id}"), $(gpu_vram_gb_at "${id}")GB)"
        sep=", "
    done
    echo "${out}"
}
...
# mg_verify_selection <selected_ids...> — verify the runtime is actually using
# the intended GPUs by checking nvidia-smi compute apps. Prints a report;
# returns 0 when every selected GPU shows activity or when no apps are listed
# yet (model may still be loading); returns 1 when a selected GPU is idle
# while OTHER GPUs are busy (possible mis-routing).
mg_verify_selection() {
    local intended="$1"   # space-separated logical ids
    mg_ensure_detection
    local apps
    apps="$(nvidia-smi --query-compute-apps=gpu_uuid,used_memory --format=csv,noheader 2>/dev/null || true)"
    if [[ -z "${apps}" ]]; then
        echo "  Note: no compute processes reported yet (model may still be loading)."
        return 0
    fi
    # Map UUIDs back to logical indices via nvidia-smi -L
    local busy_indices="" line uuid idx
    while IFS= read -r line; do
        [[ -z "${line}" ]] && continue
        uuid="$(echo "${line}" | sed 's/.*UUID: \([^)]*\).*/\1/')"
        idx=""
        while IFS= read -r l; do
            if echo "${l}" | grep -q "UUID: ${uuid})"; then
                idx="$(echo "${l}" | sed 's/^GPU \([0-9]*\):.*/\1/')"
                break
            fi
        done < <(nvidia-smi -L 2>/dev/null)
        if [[ -n "${idx}" ]]; then
            busy_indices="${busy_indices:+${busy_indices} }${idx}"
        fi
    done <<< "${apps}"

    local id active=0 missing=""
    for id in ${intended}; do
        if echo "${busy_indices}" | tr ' ' '\n' | grep -qx "${id}"; then
            active=$((active + 1))
        else
            missing="${missing:+${missing} }${id}"
        fi
    done
    if [[ "${active}" -eq "$(echo "${intended}" | wc -w | tr -d ' ')" ]]; then
        echo "  Verified: ${active} intended GPU(s) show compute activity."
        return 0
    elif [[ "${active}" -gt 0 && -n "${missing}" ]]; then
        echo "  WARNING: intended GPU(s)${missing} show no compute activity while others do."
        return 1
    fi
    echo "  Note: intended GPU(s) not yet showing compute activity."
    return 0
}
